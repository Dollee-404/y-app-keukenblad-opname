/**
 * Test-script voor taak 7: sparing-aanslag-maten (telescopisch gestapeld).
 * Toont 1307 + 1956 horizontaal onder blad, 500 verticaal links.
 *
 * Uitvoeren: npx tsx scripts/gen-test-taak7.ts
 * Output:    /tmp/kbf-pdf-test/test-taak7-sparingmaten.pdf
 */
...
import { renderBuitenmaten, renderSparingMaten } from '../src/pdf/renderMaatvoering.js';
...
doc.text('Taak 7 — Sparing-maten: "1307" + "1956" horizontaal, "500" verticaal', 15, 8);
doc.text(`Blad 1958x1001 | Bora C75 op (1307, 500) | schaal=${vp.scaleFactor.toFixed(5)}`, 15, 13);

// ── Output ────────────────────────────────────────────────────────────────────
mkdirSync('/tmp/kbf-pdf-test', { recursive: true });
const buf = Buffer.from(doc.output('arraybuffer'));
writeFileSync('/tmp/kbf-pdf-test/test-taak7-sparingmaten.pdf', buf);
console.log('✓ /tmp/kbf-pdf-test/test-taak7-sparingmaten.pdf geschreven');
