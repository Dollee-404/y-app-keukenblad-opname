...
import { IN_YAPP_CONTEXT } from "../bridge";
...
function inputKlasse() {
  return "w-full min-h-[44px] px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-teal-500";
}

function labelKlasse() {
...
export default function Step1Klant({ state, dispatch }: Props) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      setZoekFout(null);
      try {
        const resultaten = await searchCustomers(zoekQuery);
  ...
  return (
    <div className="space-y-8">
      ...
      <section className="bg-white rounded-xl border border-slate-200 p-5 space-y-4">
          ...
          <select
            ...
            onChange={(e) => {
              ...
              if (v) {
                dispatch({
                  type: "SET_VERKOPER",
                  payload: { naam: v.naam, email: v.email, telefoon: seed.bedrijf.telefoon },
                });
              }
            }}
          >
        ...
        <div>
          <label className={labelKlasse()}>Uw referentie <span className="text-slate-400 font-normal">(optioneel)</span></label>
          <input
            type="text"
            className={inputKlasse()}
                ...
                {zoekResultaten.map((k) => (
                  <li key={k.name}>
                    ...
                    >
                      ...
                      {k.email_id && (
                        <span className="block text-xs text-slate-400">{k.email_id}</span>
                      )}
                    </button>
                  </li>
                ))}
      ...
      <section className="bg-white rounded-xl border border-slate-200 p-5 space-y-4">
        ...
        <label className="flex items-center gap-3 min-h-[44px] cursor-pointer">
          <input
            ...
            onChange={(e) =>
              dispatch({ type: "SET_AFLEVER_GELIJK_AAN_OPDRACHTGEVER", payload: e.target.checked })
            }
          />
            ...
            {(["naam", "straat", "postcodePlaats"] as const).map((veld) => (
              <div key={veld}>
                ...
                <input
                  ...
                  onChange={(e) =>
                    dispatch({ type: "SET_AFLEVERADRES", payload: { [veld]: e.target.value } })
                  }
                />
              </div>
            ))}
        ...
        <label className="flex items-center gap-3 min-h-[44px] cursor-pointer">
          <input
            ...
            onChange={(e) =>
              dispatch({ type: "SET_KLANT_REGELT_LIFT", payload: e.target.checked })
            }
          />
